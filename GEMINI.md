...
*   `scripts/extract_materials.py`: A Python script to extract unique panel and banding materials from an S3D file.
*   `scripts/parse_info.py`: A Python script to parse the hexadecimal data from the INFO and SELBOX tags in S3D files.
*   `scripts/parse_krivulje.py`: A Python script to parse the DATA attribute of <KRIVULJE> tags in S3D files.
*   `scripts/parse_selbox.py`: A Python script to parse the hexadecimal data from the SELBOX tags in S3D files.
*   `scripts/decode_hex.py`: A Python script to decode a hexadecimal string into a readable format.
